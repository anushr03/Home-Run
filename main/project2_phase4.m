...
for n = 1:N
    ...
    if y(n)/y(n+1) < 0
        time_of_flight_s = t(n) % getting the corresponding value from time array
                                % in sec
        xF = x(n); % getting the corresponding value from x array
                             % in m
        vF = v; % getting the velocity at which ball lands, in m/s
    end
...
range_ft = xF*m2ft % in ft

% The maximum height, time of flight and range of the baseball are same to
% the answers found in phase3 through the Excel sheet. 

% ----- final velocity of the ball ------

final_speed_mph = vF/mph2mps % in miles/hr
Energy_lost_J = (1/2)*m*(v0^2 - vF^2)
...
title({'ECE 202, Project 2, Phase 4, Trajectory of a baseball', ...
...
for n = 1:N
    nRow = nRow + 1;
    export(nRow, :) = [t(n), x_ft(n), y_ft(n)];
end

writematrix(export, 'trajectory2.csv')
